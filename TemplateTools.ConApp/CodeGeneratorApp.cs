...
using TemplateCodeGenerator.AppLogic.Generation;

namespace TemplateTools.ConApp
{
    using Logic = TemplateCodeGenerator.AppLogic;
    ...
    internal partial class CodeGeneratorApp
    {
            ...
            while (input.Equals("x") == false)
            {
                ...
                if (Int32.TryParse(input, out var select))
                {
                    ...
                    if (select == 1)
                    {
                        ...
                        var qtProjects = Program.GetQTMusicStoreProjects(Program.UserPath).Union(new[] { solutionPath }).ToArray();
                    ...
                    {
                        ExecuteProjectBuild(solutionProperties);
                        if (select == 2)
                        {
                    ...
                    if (select == 5)
                    {
                        ExecuteProjectRun(solutionProperties, "3");
                    }
                    Thread.Sleep(700);
                }
            }
        ...
        private static string ExecuteProjectBuild(SolutionProperties solutionProperties)
        {
            var counter = 0;
        ...
        private static string ExecuteProjectRun(SolutionProperties solutionProperties, string executeArgs)
        {
            var maxWaiting = 10 * 60 * 1000;    // 10 minutes
