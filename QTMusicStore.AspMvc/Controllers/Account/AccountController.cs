...
using QTMusicStore.AspMvc.Models.Account;
    ...
    public partial class AccountController : MvcController
    {
        ...
        public async Task<IActionResult> LogoutAsync()
        {
            if (SessionWrapper.LoginSession != null)
            {
                ...
                if (handled == false)
                {
                    await AppLogic.AccountAccess.LogoutAsync(SessionWrapper.LoginSession.SessionToken).ConfigureAwait(false);
        ...
        public async Task<IActionResult> ChangePasswordAsync(ChangePasswordViewModel viewModel)
        {
            ...
            if (handled == false)
            {
                ...
                try
                {
                    await AppLogic.AccountAccess.ChangePasswordAsync(SessionWrapper.LoginSession?.SessionToken ?? string.Empty, viewModel.OldPassword, viewModel.NewPassword).ConfigureAwait(false);
                }
                catch (Exception ex)
                {
                    ViewBag.Error = ex.GetError();
        ...
        public async Task<IActionResult> ResetPasswordAsync(ResetPasswordViewModel viewModel)
        {
            ...
            try
            {
                await AppLogic.AccountAccess.ChangePasswordForAsync(SessionWrapper.SessionToken, viewModel.Email, viewModel.ConfirmPassword).ConfigureAwait(false);
            }
            catch (Exception ex)
            {
                ViewBag.Error = ex.GetError();
        ...
        private async Task ExecuteLogonAsync(LogonViewModel viewModel)
        {
            try
            {
                var loginSession = await AppLogic.AccountAccess.LogonAsync(viewModel.Email, viewModel.Password).ConfigureAwait(false);
