//@CodeCopy
//MdStart
using QTMusicStore.AspMvc.Extensions;

namespace QTMusicStore.AspMvc.Modules.Session
{
    public partial class SessionWrapper : ISessionWrapper
    {
        ...
        public bool IsSessionAlive
        {
            get
            {
                ...
                if (IsAuthenticated)
                {
                    result = Task.Run(async () => await AppLogic.AccountAccess.IsSessionAliveAsync(SessionToken).ConfigureAwait(false)).Result;
                }
                return result;
            }
        }
        public bool HasRole(string role, params string[] further)
        {
            ...
            {
                result = Task.Run(async () => await AppLogic.AccountAccess.HasRoleAsync(loginSession.SessionToken, role).ConfigureAwait(false)).Result;
                for (int i = 0; result == false && i < further.Length; i++)
                {
                    result = Task.Run(async () => await AppLogic.AccountAccess.HasRoleAsync(loginSession.SessionToken, further[i]).ConfigureAwait(false)).Result;
                }
            }
            return result;
        }
