...
namespace TemplateCodeGenerator.AppLogic.Generation
{
    internal sealed partial class AspMvcGenerator : ModelGenerator
    {
        ...
        private IEnumerable<IGeneratedItem> CreateControllers()
        {
            ...
            {
                if (CanCreate(type)
                    && QuerySetting<bool>(Common.UnitType.AppLogic, Common.ItemType.Controller, type, StaticLiterals.Generate, true.ToString())
        ...
        private IEnumerable<IGeneratedItem> CreateServices()
        {
            ...
            {
                if (CanCreate(type)
                    && QuerySetting<bool>(Common.UnitType.AppLogic, Common.ItemType.Service, type, StaticLiterals.Generate, true.ToString())
        ...
        private IGeneratedItem CreateAddServices()
        {
            ...
            {
                var generate = CanCreate(type) && QuerySetting<bool>(Common.UnitType.AppLogic, Common.ItemType.Controller, type, StaticLiterals.Generate, true.ToString())
            ...
            foreach (var type in entityProject.ServiceTypes)
            {
                var generate = CanCreate(type) && QuerySetting<bool>(Common.UnitType.AppLogic, Common.ItemType.Service, type, StaticLiterals.Generate, true.ToString())
                                               && QuerySetting<bool>(Common.ItemType.AddServices, type, StaticLiterals.Generate, GenerateAddServices.ToString());
