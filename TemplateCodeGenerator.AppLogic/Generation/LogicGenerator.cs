//@CodeCopy
//MdStart
namespace TemplateCodeGenerator.AppLogic.Generation
{
    ...
    internal sealed partial class LogicGenerator : ModelGenerator
    {
        ...
        private IGeneratedItem CreateDbContext()
        {
            ...
            var result = new Models.GeneratedItem(Common.UnitType.AppLogic, Common.ItemType.DbContext)
            {
        ...
        private IEnumerable<IGeneratedItem> CreateModels()
        {
            ...
            if (GenerateModels)
            {
                foreach (var type in entityProject.EntityTypes)
                {
                    ...
                    {
                        result.Add(CreateDelegateModelFromType(type, Common.UnitType.AppLogic, Common.ItemType.Model));
                        result.Add(CreateModelInheritance(type, Common.UnitType.AppLogic, Common.ItemType.Model));
                    }
                }
            }
            return result;
        ...
        private IEnumerable<IGeneratedItem> CreateContracts()
        {
            ...
            if (GenerateAccessContracts)
            {
                foreach (var type in entityProject.EntityTypes)
                {
                    ...
                    {
                        result.Add(CreateAccessContract(type, Common.UnitType.AppLogic, Common.ItemType.AccessContract));
                    }
                }
            }
            ...
            if (GenerateServiceContracts)
            {
                foreach (var type in entityProject.ServiceTypes)
                {
                    ...
                    {
                        result.Add(CreateServiceContract(type, Common.UnitType.AppLogic, Common.ItemType.ServiceContract));
                    }
                }
            }
            return result;
        ...
        private IEnumerable<IGeneratedItem> CreateControllers()
        {
            ...
            if (GenerateControllers)
            {
                foreach (var type in entityProject.EntityTypes)
                {
                    ...
                    {
                        result.Add(CreateControllerFromType(type, Common.UnitType.AppLogic, Common.ItemType.Controller));
                    }
                }
            }
            return result;
        ...
        private IEnumerable<IGeneratedItem> CreateServices()
        {
            ...
            if (GenerateServices)
            {
                foreach (var type in entityProject.ServiceTypes)
                {
                    ...
                    {
                        result.Add(CreateServiceFromType(type, Common.UnitType.AppLogic, Common.ItemType.Service));
                    }
                }
            }
            return result;
        ...
        private IEnumerable<IGeneratedItem> CreateFacades()
        {
            ...
            if (GenerateFacades)
            {
                foreach (var type in entityProject.EntityTypes)
                {
                    ...
                    {
                        result.Add(CreateFacadeFromType(type, Common.UnitType.AppLogic, Common.ItemType.Facade));
                    }
                }
            }
            return result;
        ...
        private IGeneratedItem CreateFactory()
        {
            ...
            var result = new Models.GeneratedItem(Common.UnitType.AppLogic, Common.ItemType.Factory)
            {
        ...
        private T QuerySetting<T>(Common.ItemType itemType, Type type, string valueName, string defaultValue)
        {
            ...
            try
            {
                result = (T)Convert.ChangeType(QueryGenerationSettingValue(Common.UnitType.AppLogic, itemType, CreateEntitiesSubTypeFromType(type), valueName, defaultValue), typeof(T));
            }
            catch (Exception ex)
            {
        ...
        private T QuerySetting<T>(Common.ItemType itemType, string itemName, string valueName, string defaultValue)
        {
            ...
            try
            {
                result = (T)Convert.ChangeType(QueryGenerationSettingValue(Common.UnitType.AppLogic, itemType, itemName, valueName, defaultValue), typeof(T));
            }
            catch (Exception ex)
            {
                result = (T)Convert.ChangeType(defaultValue, typeof(T));
